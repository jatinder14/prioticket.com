	...
	*/
		...
		public function printGraph() {
			...
			*/
			...
			if(!empty($this->Graph->axis_labels)) {	
					if($this->page="codeOverview")
					{
                                                $url .= 'chxt=x&amp;';
                                                $url .= 'chl='.$this->Graph->axis_labels[0].'&amp;';
	...
	class GoogleGraphOriginal {
		...
		*/
		...
		public function GoogleGraphOriginal() {
			/*
				Description: Initializes object
				Important Variables:
					'Graph'				=	Create Graph object
			...
			*/
			...
			if(!empty($this->Graph->line_colors))
				$url .= 'chco='.implode(',', $this->Graph->line_colors).'&';
                            ...
                            else if($this->page=="weekoverview" || $this->page=="dayoverview")
                            {
                               $url .= 'chbh=a,5,15'.'&';;
                            }
                            else if($this->page=="countryoverview")
                            {
                               $url .= 'chbh=r,0.5,1.5'.'&';;
                            }
                            else
			...
			if(!empty($this->Graph->axis_ranges)) {				
				$url .= 'chxr='.implode(',', $this->Graph->axis_ranges).'&';				
                        ...
                        if($this->page=="hoteloverview" || $this->page=="weekoverview" || $this->page=="dayoverview" ||  $this->page=="countryoverview")
                        {
                            	if(!empty($this->Graph->axis_styles)) {				
				$url .= 'chxs=';
				foreach($this->Graph->axis_styles as $axis_style) {
