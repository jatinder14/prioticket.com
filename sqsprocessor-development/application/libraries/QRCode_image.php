...
class QRCode_image {
    ...
    function makeImpl($test, $maskPattern) {
        ...
        for ($i = 0; $i < $this->moduleCount; $i++) {
            $this->modules[] = QRCode_image::createNullArray($this->moduleCount);
        }
        ...
        $data = QRCode_image::createData($this->typeNumber, $this->errorCorrectLevel, $dataArray);
    ...
    function createData($typeNumber, $errorCorrectLevel, $dataArray) {
        ...
        return QRCode_image::createBytes($buffer, $rsBlocks);
    }
    ...
    function createBytes(&$buffer, &$rsBlocks) {
        ...
        $dcdata = QRCode_image::createNullArray(count($rsBlocks) );
        $ecdata = QRCode_image::createNullArray(count($rsBlocks) );
        ...
        for ($r = 0; $r < count($rsBlocks); $r++) {
            ...
            $dcdata[$r] = QRCode_image::createNullArray($dcCount);
            ...
            $ecdata[$r] = QRCode_image::createNullArray($rsPoly->getLength() - 1);
        ...
        $data = QRCode_image::createNullArray($totalCodeCount);
